...
| `ClinikoError.swift` | Typed error set: `.unauthenticated`, `.forbidden`, `.notFound(resource:)`, `.validation(fields:)`, `.rateLimited(retryAfter:)`, `.server(status:)`, `.transport(URLError.Code)`, `.cancelled`, `.decoding(typeName:)`, `.dateMalformed`, `.nonHTTPResponse`. `.decoding(typeName:)` signals a `Decodable`-machinery failure (envelope shape, missing field, type mismatch); `.dateMalformed` (#131, split from #129) signals an ISO8601 datetime field that decoded as `String` but `ClinikoDateParser` could not parse — kept distinct so triage can tell a Cliniko schema shift from an unhandled offset/fractional variant. All payloads are structural — the whole enum is safe to interpolate at `OSLog` `.public`. |
